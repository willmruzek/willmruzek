...
export function ClientEntry({ client }: { client: ResumeClient }) {
  return (
    <div className="x:mt-(--tw-baseline-unit-value-px) x:print:mt-6">
      ...
      <ul className="x:mt-[calc(var(--tw-baseline-unit-value-px)*0.5)] x:list-disc x:space-y-[calc(var(--tw-baseline-unit-value-px)*0.5)] x:pl-4 x:text-base x:leading-(--tw-baseline-unit-value-px) x:text-gray-900 x:dark:text-gray-100 x:print:mt-3 x:print:space-y-2 x:print:text-sm x:print:leading-6">
